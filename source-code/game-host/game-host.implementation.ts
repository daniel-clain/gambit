...
export class GameHost_Implementation {
  constructor(
    private gameHostState: GameHostState,
    private updateConnectedClients: () => void
  ) {}

  connectingClientIsValid(name: string, id: string) {
  ...
  mapStateToUIState(): ServerPreGameUIState {
    return {
      ...this.gameHostState,
      connectedClients: this.gameHostState.connectedClients.map((c) => ({
        name: c.name,
        id: c.id,
      })),
      activeGames: this.gameHostState.activeGames.map((g) =>
        g.functions.getInfo()
      ),
    }
  }
  ...
  isCreateGameValid(client: ConnectedClient) {
    if (
      this.gameHostState.gamesBeingCreated.some(
  ...
  getGameBeingCreated(gameId: string): GameBeingCreated {
    return this.gameHostState.gamesBeingCreated.find((g) => g.id == gameId)!
  }

  createGamePlayersArray(gameId: string): ConnectedClient[] {
    ...
    if (gameBeingCreated.creator.name != "Game Display") {
      players.push({
        ...creator,
        socket: this.gameHostState.connectedClients.find(
          (c) => c.id == creator.id
        )!.socket,
      })
    }
  ...
  createGameDisplaysArray(gameId: string): ConnectedClient[] {
    const gameBeingCreated: GameBeingCreated = this.getGameBeingCreated(gameId)
  ...
  gameHandlePlayerDisconnect(client: ConnectedClient) {
    this.gameHostState.activeGames.find((game) => {
      ...
      if (
        ...
        !connectionManager.disconnectedPlayerVotes.find(
          (d) => d.disconnectedPlayer.id == client.id
        )
      ) {
  ...
  tearDownGame = (gameId: string) => {
    const index = this.gameHostState.activeGames.findIndex(
      (g) => g.has.id == gameId
    )
  ...
  isJoinGameValid(
    ...
    gameBeingCreated: GameBeingCreated
  ): boolean {
    ...
    if (
      gameBeingCreated.clients.some(
        (joinedClient: JoinedClient) => joinedClient.id == client.id
      )
    )
  ...
  isCancelGameValid(client: ConnectedClient, gameId: string) {
    const gameBeingCreated = this.gameHostState.gamesBeingCreated.find(
      (g) => g.id == gameId
    )
    if (!gameBeingCreated)
