...
export class GameHost {
  ...
  constructor(webSocketServer: Server) {
    ...
    this.i = new GameHost_Implementation(
      this.state,
      this.updateConnectedClients
    )
  }
  ...
  updateConnectedClients = () => {
    try {
      const preGameUIState = this.i.mapStateToUIState()
      console.log("sending state to clients")
      this.state.connectedClients.forEach((c) =>
        c.socket!.emit("To Client From Server - Lobby Ui", preGameUIState)
      )
    } catch (e) {
      console.log("e :>> ", e)
    }
    ...
    const messagesFromClientToHost: FromClientToHost = {
      ...
      create: () => {
        if (this.i.isCreateGameValid(client)) {
          this.state.gamesBeingCreated.push({
            ...
            creator: new JoinedClient(client.name, client.id),
            clients: [],
          })
      ...
      cancel: (gameId) => {
        if (this.i.isCancelGameValid(client, gameId)) {
          this.state.gamesBeingCreated = this.state.gamesBeingCreated.filter(
            (g) => g.id != gameId
          )
      ...
      start: (gameId) => {
        ...
        this.state.gamesBeingCreated = gamesBeingCreated.filter(
          (g) => g.id != gameId
        )
      },
      join: (gameId) => {
        const gameBeingCreated = this.state.gamesBeingCreated.find(
          (g) => g.id == gameId
        )!
        if (this.i.isJoinGameValid(client, gameBeingCreated)) {
          gameBeingCreated.clients.push(
            new JoinedClient(client.name, client.id)
          )
          console.log(
            `${client.name} joined ${gameBeingCreated.creator.name}'s game`
          )
        }
      },
        ...
        this.state.gamesBeingCreated.find((g) => {
          if (g.id == gameId) {
            ...
            g.clients.find((c) => {
              ...
              console.log(
                `${client.name} set his ready state to ${c.ready} for ${g.creator.name}'s game lobby`
              )
            })
      ...
      leave: (gameId) => {
        this.state.gamesBeingCreated.find((g) => {
          if (g.id == gameId) {
            ...
            console.log(
              `${client.name} has left ${g.creator.name}'s game lobby`
            )
          }
        })
      },
      ...
      reset: (client) => {
        ...
        console.log(
          `${client.name} has reset, removing ${client.name} from the connected clients list`
        )

        connectedClients.splice(
          connectedClients.findIndex((c) => c.id == client.id),
          1
        )
      },
      disconnect: (reason) => {
        ...
        console.log(
          `${client.name} disconnected due to ${reason}, removing ${client.name} from the connected clients list`
        )
        ...
        gamesBeingCreated.find((g) => {
          ...
          if (g.clients.some((c) => c.id == client.id))
            messagesFromClientToHost.leave(g.id)
        })
  ...
  tearDownGame = (gameId: string) => this.i.tearDownGame(gameId)
}
