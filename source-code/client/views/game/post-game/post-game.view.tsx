...
export const PostGame_View = observer(() => {
              ...
              {player.fighters.map((fighter) => (
                <players-fighter key={fighter.name}>
                  ...
                  <bottom-block>
                    ...
                    <right-block>
                      <Fights numberOfFights={fighter.stats.numberOfFights} />
                      <Wins numberOfWins={fighter.stats.numberOfWins} />
  ...
  function getModelImageClass(fighter: FighterInfo) {
    ...
    const fitness = fighter.stats.fitness
    let skinName: Skin
  ...
  function getInfoListData(fighter: FighterInfo): InfoBoxListItem[] {
    return (
      ["strength", "fitness", "intelligence", "aggression"] as Array<
        keyof FighterStatsInfo
      >
    ).map((stat) => ({
      label: stat,
