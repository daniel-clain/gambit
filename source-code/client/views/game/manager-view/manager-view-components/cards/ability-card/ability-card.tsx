...
import { KnownFighterInfo } from "../../../../../../../interfaces/front-end-state-interface"
import { InfoBoxListItem } from "../../../../../../../interfaces/game/info-box-list"
import {
  Evidence,
  IllegalActivityName_Set,
} from "../../../../../../../types/game/evidence.type"
...
import {
  ...
  showFighter,
  showManager,
} from "../../../../../../front-end-service/front-end-service"
...
export const AbilityCard = observer(() => {
  ...
  const { enoughFightersForFinalTournament, playerManagerUIState } =
    serverGameUIState!

  const { managerInfo, delayedExecutionAbilities, jobSeekers, week } =
  ...
  const clientAbility: ClientAbility = abilities.find(
    (ability) => ability.name == activeAbility.name
  )!
  ...
  if (name == "Dope Fighter" || name == "Poison Fighter") {
    abilityRequirementsInfo.push({
      label: "Requires:",
      value: "Must have a drug dealer working for you",
    })
  }

  const noNeedForTargetSelect: boolean =
    clientAbility.isValidTarget == undefined
    ...
    <Modal>
      <div className="ability-card card">
        ...
        <div className="card__two-columns">
          <div
            className={`card__two-columns__left ability-card__image ${
              "ability-card__image__" +
              activeAbility.name.toLocaleLowerCase().split(" ").join("-")
            }`}
          ></div>
          <div className="card__two-columns__right">
            <InfoBox heading={"Ability Info"} info={longDescription} />
          ...
          <div className="ability-card__variables">
            {noNeedForTargetSelect ? (
              ...
              <div className="target">
                ...
                {target ? (
                  ...
                  <div className="target__value target__value--required">
                    target required
                  </div>
                )}
                <button
            ...
            <div className="source">
              ...
              {source ? (
                ...
                <div className="source__value source__value--required">
                  source required
                </div>
              )}
              <button
            ...
            {abilityData.name == "Prosecute Manager" && activeAbility.target ? (
              <div className="evidence-items-container">
                {Array.from(IllegalActivityName_Set).map((illegalActivity) => {
                  return (
                    <div className="illegal-activity" key={illegalActivity}>
                      <div className="illegal-activity__name">
                        {illegalActivity}
                      </div>
                      <div className="illegal-activity__evidence-items">
                        {managerInfo.evidence
                          .filter(
                            (e) => e.managerName == activeAbility.target?.name
                          )
                          ...
                          .map((evidence, index) => (
                            <CheckboxItem
                              ...
                              onSelected={() =>
                                onEvidenceToggle(evidence, index)
                              }
        ...
        {activeAbility.name == "Offer Contract" ? (
          <OfferContractPartial
            {...{ activeAbility, setActiveAbility, jobSeekers, managerInfo }}
          />
        ) : (
          ""
        )}
        ...
        <button
          className="standard-button ability-card__button"
          onClick={confirmButtonClicked.bind(this)}
        >
          Confirm
        </button>
  ...
  function isEvidenceSelected(evidence: Evidence, index: number) {
    ...
    return !!evidenceItems?.find(
      (e) => e.id == evidence.illegalActivity + evidence.managerName + index
    )
  }
  ...
  function confirmButtonClicked() {
    ...
    if (activeAbility.name == "Research Fighter") {
      ...
      const fighter: KnownFighterInfo = knownFighters.find(
        (f) => f.name == target!.name
      )!

      showFighter(fighter.name)
    }
    if (activeAbility.name == "Investigate Manager") {
      let knownManager = managerInfo.otherManagers.find(
  ...
  function onEvidenceToggle(evidence: Evidence, index: number) {
    ...
    const evidenceItems = activeAbility.additionalData as DataEvidence[]
    const evidenceAlreadyChecked = evidenceItems?.find(
      (e) => e.id == evidenceId
    )
    if (evidenceAlreadyChecked) {
      setActiveAbility({
        ...activeAbility,
        additionalData: [...evidenceItems?.filter((e) => e.id != evidenceId)],
      })
    } else {
      ...
      const checkedEvidence: DataEvidence[] = activeAbility.additionalData
        ? [...evidenceItems, evidenceObj]
        : [evidenceObj]
