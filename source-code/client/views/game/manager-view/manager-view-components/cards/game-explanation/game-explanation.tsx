...
import {
  ...
  wealthVictoryClient,
} from "../../../../../../client-abilities/client-abilities"
...
console.log("gameConfiguration", gameConfiguration)
...
const infoItems: Info[] = [
  {
    description:
      ...
      `\n\t\t - when a manager runs out of money, they have no choice but to go bankrupt` +
      `\n\n Sinister Victory` +
      `\n\t\t - ${sinisterVictoryClient.longDescription}` +
      `\n\n Domination Victory` +
      `\n\t\t - ${dominationVictoryClient.longDescription}` +
      `\n\n Wealth Victory` +
      `\n\t\t - ${wealthVictoryClient.longDescription}`,
    categories: ["How To Win"],
  },
  {
    description:
      "When strength or fitness is over 10 it goes down a little bit each week until back to 10",
    categories: ["Strength", "Fitness"],
  },
  {
    description:
      "A lawyer need evidence in order to prosecute another manager. Evidence of illegal activity is obtained through the private agent doing surveillance on a fighter or manager",
    categories: ["Illegal Actions", "Lawyer", "Private Agent", "Surveillance"],
  },
  {
    description:
      "Your opponent managers have hidden information and hidden actions, unless you use a private agent. A private agent can investigate a manager to find out their money, fighters, employees, loan, evidence and activity logs",
    categories: ["Investigate Manager", "Manager", "Private Agent"],
  },
  {
    description:
      "The player represents the manager in the game, your goal is to make money and outlast each other manager",
    categories: ["Manager"],
  },
  {
    description:
      `The way to make money in the game is betting on the winning fighter. Winnings are calculated like:` +
      `\n\t base ammount (${betWinningsBase})` +
      `\n\t plus(+) your bet times(*) bet multiplier (${betPercentageIncreased}%)` +
      `\n\t plus(+) the publicity rating of each fighter` +
      `\n\t all increased by ${numFightersPercentMultiplier}% for each fighter in the fight` +
      `\n\n` +
      `If the winning fighter is your fighter then more is added before the number of fighters multiplier:` +
      `\n\t your fighter base (${playersFighterWinBase}) times(*) their publicity rating`,
    categories: ["Money", "Bet", "Your Fighter"],
  },
  {
    description:
      "Talent scout frees up your manager action points. Talent scout can research fighters and offer contracts. Skilled talent scout finds more info than the manager",
    categories: ["Talent Scout"],
  },
  {
    description:
      "You can train fighters, each time you train, their strength or fitness will go up by 0.5 for each skill point of the trainer",
  ...
  {
    description:
      "When you research, the number next to the value is the weeks since learned, and it may not reflect the true value",
    categories: ["Talent Scout", "Fighter", "Research"],
  },
  {
    description:
      "Recontract your fighter on the last week of contract or they will go back in the job seeker pool",
    categories: ["Fighter", "Contract"],
  },
  {
    description:
      "When a lawyer successfully prosecutes you, you go to jail, which mean you can not use manager action points for the duration ",
    categories: ["Lawyer", "Jail", "Action Points"],
  },
  {
    description:
      "Your employees will remain with you until their contract expires. Only fighters can be recontracted",
  ...
  {
    description:
      `When your fighter wins, you will make money even if you did not bet on him. ` +
      `\n\t your fighter base (${playersFighterWinBase}) times(*) their publicity rating`,
  ...
  {
    description: `When a fighter lands a critical strike or gets attacked when on high spirits, theres is a chance it will trigger a rampage. When rampaging, fighter does not take defensive actions, and his speed and aggression are increased and is no affected by low energy. Chance to rampage is higher if the fighter has high aggression and spirit, and lower if low energy or stamina`,
      ...
      <div className="card game-explanation-card">
        ...
        <div className="card__two-columns">
          <div className="card__two-columns__left categories">
            {sortedCategories.map((category) => (
              <div
                key={category}
                className={`category ${
                  category == selectedCategory ? "category--selected" : ""
                }`}
          ...
          <div className="card__two-columns__right info-items">
            {infoItems
              .filter((i) => i.categories.some((c) => c == selectedCategory))
              .map((i) => (
                <div className="info-item" key={i.description}>
                  ...
                  <div className="info-item__description">{i.description}</div>
                </div>
              ))}
          </div>
        </div>
