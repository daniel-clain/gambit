...
export const DisconnectedPlayerModal = () => {
  const {
    ...
    clientUIState: {
      clientPreGameUIState: { clientId, clientName },
    },
  } = frontEndState
  ...
  const votesByPlayer: VotesByPlayer[] =
    disconnectedPlayerVotes[0].playerVotesToDrop.map(
      ({ votingPlayer }): VotesByPlayer => ({
        votingPlayer,
        disconnectedPlayersVotes: disconnectedPlayerVotes.map((d) => ({
          disconnectedPlayer: d.disconnectedPlayer,
          voteToDrop: d.playerVotesToDrop.find(
            (v) => v.votingPlayer.id == votingPlayer.id
          )!.drop,
        })),
      })
    )
    ...
    <div className="disconnected-player-modal">
      ...
      <div className="modal-content">
        ...
        <div className="details">
          The following player(s) have disconnected, wait for them to reconnect
          or vote to drop the player and continue without them.
        </div>
          ...
          {votesByPlayer.map(({ votingPlayer, disconnectedPlayersVotes }) => (
            <tr
              ...
              className={`
                player-vote-row 
                ${
                  votingPlayer.id == player.id
                    ? "player-vote-row--this-player"
                    : ""
                }
              `}
            >
              <td className={`player-vote`}>{votingPlayer.name}'s vote:</td>

              {disconnectedPlayersVotes.map(
                ({ disconnectedPlayer, voteToDrop }) => (
                  <td
