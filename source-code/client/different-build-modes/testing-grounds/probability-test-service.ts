...
export const fight = new Fight([testFighter, inFront, behind])
...
export function initialSetup() {
  ...
  function setBaseStats() {
    fight.fighters.forEach(({ fighting }) => {
      ...
      fighting.otherFightersInFight = fight.fighters.filter(
        (fighter) => fighter.name != fighting.fighter.name
      )
...
export function update() {
  runInAction(() => {
    ...
    frontEndState.serverUIState = {
      serverGameUIState: {
        fightUIState: fight.fightUiState,
...
export function getProbabilities() {
  const probabilities = fight.fighters
    .find((f) => f.name == "TestFighter")
    .fighting.actions.getActionProbabilities()
  console.log(probabilities)
}

export function setMemoryOfBehindElapsed(val: number) {
  const timer = testFighter.fighting.timers.isActive("memory of enemy behind")
  timer.cancel()
