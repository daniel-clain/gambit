...
function getClosestCoordsOnAnEdgeFromAPoint(edgeName: Edge, point: Coords): Coords{
  const {point1, point2} = edges[edgeName]
  return getClosestCoordsOnASlopeFromAPoint({point1, point2}, point)
}

function getClosestCoordsOnASlopeFromAPoint(slopePoints: {point1: Coords, point2: Coords}, point: Coords): Coords{
...
function isPointOutsideOfEdge(edgeName: Edge, point: Coords): boolean {
  const closestPointOnSlope: Coords = getClosestCoordsOnAnEdgeFromAPoint(edgeName, point)
...
function getClosestEdge(point: Coords): Edge{
  return edgeKeys.reduce(
    (closestEdge: {edgeName: Edge, distance: number} | undefined, edgeName) => {
      const distance = getDistanceFromEdge(edgeName, point)
      if(!closestEdge || distance < closestEdge.distance)
        return closestEdge = {edgeName, distance}
    }, undefined
  )!.edgeName
}
