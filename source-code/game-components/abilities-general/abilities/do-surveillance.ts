...
import { Ability, AbilityData, AbilityName, ServerAbility } from "../ability"
  ...
  execute(abilityData: AbilityData, game: Game) {
    ...
    const fighter =
      (target!.characterType == "Fighter" &&
        fighters.find((f) => f.name == target!.name)) ||
      undefined

    const manager =
      (target!.characterType == "Known Manager" &&
        managers.find((m) => m.has.name == target!.name)) ||
      undefined

    ;(fighter || manager)!.state.underSurveillance = {
...
type HandleSurveillanceProps = {
  surveilledManager?: Manager
  surveilledFighter?: Fighter
  abilityData: AbilityData
  game: Game
}

export const handleUnderSurveillance = ({
  surveilledManager,
  surveilledFighter,
  abilityData,
  game,
}: HandleSurveillanceProps) => {
  ...
  const privateAgentName =
    surveilledManager?.state.underSurveillance!.professional ||
    surveilledFighter?.state.underSurveillance!.professional

  const managerDoingSurveillance = game.has.managers.find((m) =>
    m.has.employees.find((e) => e.name == privateAgentName)
  )!

  const agentDoingSurveillance = managerDoingSurveillance.has.employees.find(
  ...
  if (surveilledManager && willDiscover) {
    const evidenceDescription = `(Week ${weekNumber}) - ${
      surveilledManager.has.name
    } ${
      source!.characterType != "Manager"
        ? `'s ${source!.characterType} ${source!.name}`
        : ""
    } was seen using ability ${name} ${
      target ? `targeting ${target.characterType} ${target.name}` : ""
    }`

    managerDoingSurveillance.has.evidence.push({
      id: Number(randomNumberDigits(8)),
      managerName: surveilledManager.has.name,
      abilityData,
      illegalActivity,
      evidenceDescription,
    })
    ...
    managerDoingSurveillance.functions.addToLog({
      ...
      message: `Your private agent ${privateAgentName} has caught ${
        source!.name
      } doing "${name}" ${
        source!.characterType != "Manager"
          ? `. ${source!.name}  works for ${surveilledManager.has.name}`
          : ""
      }. You have stored this information away as evidence`,
    })
  }
  ...
  if (surveilledFighter && willDiscover) {
    ...
    managerDoingSurveillance.has.evidence.push({
      id: randomNumberDigits(8),
      managerName: sourceManager.has.name,
      abilityData,
      illegalActivity,
      evidenceDescription,
    })
    ...
    managerDoingSurveillance.functions.addToLog({
      ...
      message: `Your private agent ${privateAgentName} has caught ${
        ...
        source?.characterType != "Manager"
          ? `. ${source!.name}  works for ${surveilledFighter.name}`
          : ""
      }. You have stored this information away as evidence`,
    })
  }
