...
export const offerContractServer: ServerAbility = {
  execute(abilityData: AbilityData, game: Game) {
    ...
    const contractOffer: ContractOffer =
      abilityData.additionalData.contractOffer

    const jobSeeker: JobSeeker = game.has.weekController.jobSeekers.find(
    ...
    if (jobSeeker) {
      if (jobSeeker.type == "Professional") {
        ...
        const jobSeekerIndex = game.has.weekController.jobSeekers.findIndex(
          (jobSeeker) => jobSeeker.name == target!.name
        )
      ...
      if (jobSeeker.type == "Fighter") {
        const fighter: Fighter = game.has.fighters.find(
          (fighter) => fighter.name == jobSeeker.name
        )!
        if (fighter.state.dead) {
          sourceManager.functions.addToLog({
            weekNumber,
            message: `Offer contract to ${
              target!.name
            } failed because he was murdered`,
        ...
        const knownFighterIndex = sourceManager.has.knownFighters.findIndex(
    ...
    if (isRecontractingFighter) {
      const fighter = sourceManager.has.fighters.find(
        (fighter) => fighter.name == target!.name
      )
      if (fighter) {
        ...
        fighter.state.goalContract = undefined
      }
    }

    sourceManager.functions.addToLog({
      weekNumber,
      message: `${target!.name} has agreed to the contract offed.`,
      type: "employee outcome",
    })
