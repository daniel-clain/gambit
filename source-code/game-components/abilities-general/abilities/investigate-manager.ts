...
export const investigateManagerServer: ServerAbility = {
  execute(abilityData: AbilityData, game: Game){
    ...
    const privateAgent = sourceManager.has.employees.find(employee => employee.name == source!.name)!
    ...
    sourceManager.functions.addToLog({
      weekNumber,
      message: `${source!.characterType} ${source!.name} used ${abilityData.name} and found out the following stats about ${target!.name}: \n ${toWrittenList(discoveredStats.map(s => s.key))}`});
