...
export const murderFighterServer: ServerAbility = {
  execute(abilityData: AbilityData, game: Game) {
    ...
    const hitman = sourceManager.has.employees.find(
    ...
    if (guardLevel > 0) {
      ...
      const randomNum = randomNumber({ to: 100 })
      if (randomNum < 65 + hitman.skillLevel * 5) success = true
    }
    if (success) {
      ...
      if (fightersManager)
        fightersManager.functions.addToLog({
          weekNumber,
          type: "critical",
          message: `Your fighter, ${fighter.name}, has been murdered`,
        })
      ...
      sourceManager.functions.addToLog({
        weekNumber,
        type: "employee outcome",
        message: `Hitman ${hitman.name} failed to murder target fighter ${
          target!.name
