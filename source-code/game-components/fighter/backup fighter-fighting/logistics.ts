...
export default class Logistics {
  ...
  justDodged: boolean
  justTookHit: boolean
  rememberedEnemyBehind: Fighter | undefined
  ...
  getHasDefendOpportunity(enemy: Fighter): boolean {
    ...
    return !!canDefendActions.find(
      (a) => a == actions.currentInterruptibleAction
    )
  }
  ...
  get closestRememberedEnemy(): Fighter {
    ...
    if (!this.closestEnemyInFront && !enemyBehind) return
    if (this.closestEnemyInFront && !enemyBehind)
      return this.closestEnemyInFront
    if (!this.closestEnemyInFront && enemyBehind) return enemyBehind
  ...
  enemyIsOnARampage(enemy: Fighter): boolean {
    if (!enemy) return
    return enemy.fighting.timers.activeTimers.some(
      (timer) => timer.name == "on a rampage"
    )
  }
