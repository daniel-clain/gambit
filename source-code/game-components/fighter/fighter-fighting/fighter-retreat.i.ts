...
import { Edge, EdgeDistance } from "../../../types/fighter/edge"
...
export const fighterRetreatImplementation = (fighting: FighterFighting) => {
  ...
  function getDirectionAlongEdgePastFlanker(): Angle | undefined {
    const [flanker1Data, flanker2Data] = logistics.flanked!.flankers.map(
      (flanker) => {
        const threatLevel = logistics.getEnemyThreatPercentage(flanker)
        const directionFromFighter = getDirectionAwayFromEnemy(flanker)
        const directionAlongEdge = getIsInCorner()
          ? getCorneredDirectionAlongEdgeAwayFromEnemy(flanker)
          : getDirectionAlongEdgeAwayFromEnemy(flanker)
        return {
          directionAlongEdge,
          threatLevel,
          directionFromFighter,
          flanker,
        }
      }
    )

    const directionFromFlanker1Blocked = enemyBlockingDirection(
      flanker2Data.flanker,
      flanker1Data.directionAlongEdge
    )
    const directionFromFlanker2Blocked = enemyBlockingDirection(
      flanker1Data.flanker,
      flanker2Data.directionAlongEdge
    )

    if (directionFromFlanker1Blocked && directionFromFlanker2Blocked) return

    if (directionFromFlanker1Blocked) return flanker2Data.directionAlongEdge

    if (directionFromFlanker2Blocked) return flanker1Data.directionAlongEdge

    return flanker1Data.threatLevel > flanker2Data.threatLevel
      ? flanker2Data.directionAlongEdge
      : flanker1Data.directionAlongEdge
  }
  ...
  function getFlankerThreatInfluence(): [Percent, Percent] {
    ...
    const [flanker1Threat, flanker2Threat] = flankers.map(
      logistics.getEnemyThreatPercentage.bind(logistics)
    )
    const totalThreatValue = flanker1Threat + flanker2Threat

    const [flanker1Influence, flanker2Influence] = [
      flanker1Threat,
      flanker2Threat,
    ].map((t) => toPercent(t / totalThreatValue))
  ...
  function getDirectionBetweenFlankers(): Angle {
    ...
    const [directionOfFlanker1, directionOfFlanker2] = flankers.map((f) =>
      proximity.getDirectionOfEnemyCenterPoint(f)
    )
  ...
  function getDirectionFromFlanked() {
    ...
    const [directionFromFlanker1, directionFromFlanker2] = flankers.map(
  ...
  function getNearEdgeInDirection(retreatDirection: Angle): Edge | undefined {
    ...
    const edgeInDirection = nearEdges.find(({ edge }) => {
      const edgeCoords = octagon.getClosestCoordsOnAnEdgeFromAPoint(
        edge,
        movement.coords
      )
      const directionOfEdge = getDirectionOfPosition1ToPosition2(
        movement.coords,
        edgeCoords
      )
      const directionTowardEdge = directionWithinDegreesOfDirection(
        retreatDirection,
        45,
        directionOfEdge
      )
  ...
  function getDirectionInfluencedByNearEdge(
    initialDirection: Angle,
    nearEdgeInDirection: Edge
  ) {
    ...
    const directionOfCorner = getDirectionOfEdgeCornerClosestToDirection(
      nearEdgeInDirection,
      initialDirection
    )
  ...
  function getDirectionAlongEdgeClosestToDirection(direction: Angle): Angle {
    ...
    const point2AngleDiff = getSmallestAngleBetween2Directions(
      edgeDirection2,
      direction
    ).angleBetween
  ...
  function getDirectionAlongEdgeToCornerFurthestFromEnemy(): Angle {
    const enemyCoords =
      logistics.closestRememberedEnemy!.fighting.movement.coords
    if (getIsInCorner()) {
      const cornerEdges = proximity.getEdges(maxAgainstEdgeDistance)

      const cornerPoint = getCornerPoint([
        cornerEdges[0].edge,
        cornerEdges[1].edge,
      ])
      ...
      const { furthestPoint, edgeDistance } = cornerEdges.reduce(
        (selected, edgeDistance) => {
          const [point1, point2]: Coords[] = Object(
            octagon.edges[edgeDistance.edge]
          ).values
          const [distance1, distance2] = [point1, point2].map((point) =>
            getDistanceBetweenTwoPoints(point, enemyCoords)
          )

          const [furthestPoint, furthestPointDistance] =
            distance1 > distance2 ? [point1, distance1] : [point2, distance2]
      ...
      const directionAlongEdge = getDirectionOfPosition1ToPosition2(
        cornerPoint,
        furthestPoint
      )
  ...
  function getCorneredDirectionAlongEdgeAwayFromEnemy(enemy: Fighter): Angle {
    const furthestEdgeFromEnemy = getEdgeFurthestFromEnemy(
      getCornerEdges(),
      enemy
    )

    const directionAlongEdge = getDirectionAlongEdgeToFurthestCorner(
      furthestEdgeFromEnemy
    )
    return directionAlongEdge
  }
  ...
  function getIsDirectionIntoEdge(direction: Angle) {
    if (getIsAgainstEdge()) {
      if (getIsInCorner()) {
        ...
        const [edge1Directions, edge2Directions] = [edge1, edge2].map(
          getDirectionsAlongEdge
        )
        ...
        if (edge1Directions.some((x) => x == direction)) {
          ...
          return directionWithinDegreesOfDirection(
            direction,
            90,
            directionOfEdge
          )
  ...
  function getDirectionAlongEdgeToFurthestCorner(edge: Edge): Angle {
    const { point1, point2 } = octagon.edges[edge]
    const [distanceFromPoint1, distanceFromPoint2] = [point1, point2].map(
      (point) => getDistanceBetweenTwoPoints(movement.coords, point)
    )

    const [furthestPoint, closestPoint] =
      distanceFromPoint1 > distanceFromPoint2
        ? [point1, point2]
        : [point2, point1]
  ...
  function getDirectionOfEdgeCornerClosestToDirection(
    edge: Edge,
    retreatDirection: Angle
  ): Angle {
    const [point1Data, point2Data] = Object.values(octagon.edges[edge]).map(
      (corner: Coords) => {
        const cornerDirection = getDirectionOfPosition1ToPosition2(
          movement.coords,
          corner
        )
        const angleDiff = getSmallestAngleBetween2Directions(
          cornerDirection,
          retreatDirection
        ).angleBetween
  ...
  function getCornerEdges(): [Edge, Edge] {
    const [edge1, edge2] = proximity
      .getEdges(maxCornerDistance)
      .map(({ edge }) => edge)
  ...
  function getDirectionBetweenBasedOnInfluence(
    ...
    const { angleBetween, crosses0 } = getSmallestAngleBetween2Directions(
      direction1,
      direction2
    )
    ...
    const direction1AngleInfluenceAmount = toAngle(
      angleBetween * direction1Influence
    )
    const direction2AngleInfluenceAmount = toAngle(
      angleBetween * direction2Influence
    )
    ...
    if (influenceRepulseOrAttract == "attract") {
      if (direction1 == leftAngle)
        angle = subtractAngle2FromAngle1(
          rightAngle,
          direction1AngleInfluenceAmount
        )

      if (direction2 == leftAngle)
        angle = subtractAngle2FromAngle1(
          rightAngle,
          direction2AngleInfluenceAmount
        )
    } else if (influenceRepulseOrAttract == "repulse") {
      if (direction1 == leftAngle)
        angle = add2Angles(leftAngle, direction1AngleInfluenceAmount)

      if (direction2 == leftAngle)
        angle = add2Angles(leftAngle, direction2AngleInfluenceAmount)
    }
  ...
  function enemyBlockingDirection(enemy: Fighter, direction: Angle): boolean {
    ...
    if (closeness == Closeness["striking range"]) blocking = true
    if (closeness == Closeness["close"])
      blocking = directionWithinDegreesOfDirection(
        direction,
        70,
        directionFromEnemy
      )
