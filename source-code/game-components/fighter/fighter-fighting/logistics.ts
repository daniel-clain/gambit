...
export default class Logistics {
  ...
  get timeSinceLastCombat() {
    ...
    return timers.get("last combat action").elapsedTime!
  }
  ...
  get flanked(): Flanked | undefined {
    ...
    if (!this.rememberedEnemyBehind || !this.closestEnemyInFront) return

    if (this.closestEnemyInFront.name == this.rememberedEnemyBehind.name) {
      debugger
    }

    const enemy2Closeness = proximity.getEnemyCombatCloseness(
      this.rememberedEnemyBehind
    )
    const enemy1Closeness = proximity.getEnemyCombatCloseness(
  ...
  get highestThreatEnemy(): Fighter | undefined {
    ...
    if (!enemyInFront || !enemyBehind) return

    if (!enemyBehind) return enemyInFront

    if (!enemyInFront) return enemyBehind

    const inFrontDistance = getDistanceOfEnemyStrikingCenter(
      enemyInFront,
      fighter
    )
    const behindDistance = getDistanceOfEnemyStrikingCenter(
      enemyBehind,
      fighter
    )

    const inFrontThreat = this.isEnemyAttacking(enemyInFront)
      ? inFrontDistance
      : inFrontDistance * 0.3
    const behindThreat = this.isEnemyAttacking(enemyBehind)
      ? behindDistance
      : behindDistance * 0.3
  ...
  get onARampage(): boolean {
    const { timers } = this.fighting
    return timers.isActive("on a rampage")
  }
  ...
  get enemiesInFront() {
    ...
    const fightersInFront: Fighter[] = this.otherFightersStillFighting.filter(
      (fighter: Fighter) => {
  ...
  get otherFightersStillFighting(): Fighter[] {
    return this.fighting.otherFightersInFight.filter(
  ...
  get hasLowStamina(): boolean {
    const { stamina, stats } = this.fighting
    return stamina < stats.maxStamina * 0.5
  }

  get hasLowSpirit(): boolean {
    const { spirit, stats } = this.fighting
    return spirit < stats.maxSpirit * 0.5
  }
  ...
  getEnemyThreatPercentage(enemy: Fighter): Percent {
    ...
    const distancePercentage = toPercent(
      (distance - strikingRange) / (farRange - strikingRange)
    )
    const closenessExponentialFactor = isAttacking ? 1.6 : 1.3

    const exponentialDistancePercentage = toPercent(
      Math.pow(distancePercentage, closenessExponentialFactor)
    )

    const inverseExponentialDistancePercentage = toPercent(
      1 - exponentialDistancePercentage
    )
  ...
  getHasDefendOpportunity(enemy: Fighter): boolean {
    ...
    if (isFacingAwayFromEnemy(enemy, fighter)) return false
    return !!canDefendActions.find((a) => a == this.fighting.getCurrentAction())
  }

  get closestEnemyInFront(): Fighter | undefined {
    ...
    const closestEnemy = otherFighters.reduce(
      (selectedFighter: Fighter | undefined, loopFighter) => {
        const loopFighterLeft = loopFighter.fighting.movement.coords.x
        const loopFighterDistance =
          proximity.getDistanceFromEnemyCenterPoint(loopFighter)
        if (!selectedFighter) {
          ...
          const selectedFighterDistance =
            proximity.getDistanceFromEnemyCenterPoint(selectedFighter)
          if (facingDirection == "left") {
            if (loopFighterLeft < movement.coords.x) {
              const loopFighterCloser =
                loopFighterDistance < selectedFighterDistance
    ...
    if (closestEnemy && closestEnemy.name == this.rememberedEnemyBehind?.name) {
      console.log(
        "closest enemy infront should not also be remembered enemy behind"
      )
      debugger
    }
    return closestEnemy
  ...
  get closestRememberedEnemy(): Fighter | undefined {
    const { proximity } = this.fighting
    let enemyBehind = this.rememberedEnemyBehind
    if (!this.closestEnemyInFront && !enemyBehind) {
      return undefined
    } else if (this.closestEnemyInFront && !enemyBehind)
      return this.closestEnemyInFront
    else if (!this.closestEnemyInFront && enemyBehind) return enemyBehind
    else if (this.closestEnemyInFront && enemyBehind) {
      const inFrontEnemyDistance = proximity.getDistanceFromEnemyCenterPoint(
        this.closestEnemyInFront
      )

      const behindEnemyDistance =
        proximity.getDistanceFromEnemyCenterPoint(enemyBehind)

      const closestEnemy =
        inFrontEnemyDistance < behindEnemyDistance
          ? this.closestEnemyInFront
          : enemyBehind
  ...
  get allOtherFightersAreKnockedOut(): boolean {
    return this.fighting.otherFightersInFight.every(
      (fighter: Fighter) => fighter.fighting.knockedOut
    )
  }

  get enemyIsBehind() {
    return (
      this.closestRememberedEnemy &&
      !this.isFacingEnemy(this.closestRememberedEnemy)
    )
  }
  ...
  enemyIsOnARampage(enemy: Fighter) {
    if (!enemy) return
    return enemy.fighting.timers.activeTimers["on a rampage"].active
  }

  enemyHasLowSpirit(enemy: Fighter) {
    if (!enemy) return
  ...
  enemyHasLowStamina(enemy: Fighter) {
    if (!enemy) return
  ...
  isEnemyAttacking(enemy: Fighter) {
    if (!enemy) return
  ...
  hasRetreatOpportunity(enemy: Fighter) {
    if (!enemy) return
