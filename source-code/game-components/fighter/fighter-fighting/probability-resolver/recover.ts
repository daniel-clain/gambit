...
export const getProbabilityToRecover = (fighting: FighterFighting): number => {
  ...
  const invalid: boolean = logistics.hasFullStamina || logistics.onARampage
  ...
  if (
    enemyInFront &&
    enemyBehind &&
    enemyInFrontCloseness! > Closeness["close"] &&
  ...
  if (logistics.justTookHit) probability += 20 - spirit * 2
  ...
  function inFrontFarAndBehindFar() {
    ...
    if (lowStamina) probability += 6 + intelligence * 3

    if (lowSpirit) probability += 6 + intelligence * 3

    if (notFullStamina) probability += intelligence * 3

    if (notFullSpirit) probability += intelligence * 3

    if (hasRetreatOpportunityFromInFront) probability += intelligence * 3
    if (hasRetreatOpportunityFromBehind) probability += intelligence * 3
  }

  function inFrontNearAndBehindFar() {
    //console.log('inFrontNearAndBehindFar');
    if (lowStamina) probability += 4 + intelligence * 2

    if (lowSpirit) probability += 4 + intelligence * 2

    if (notFullStamina) probability += intelligence * 2

    if (notFullSpirit) probability += intelligence * 2

    if (hasRetreatOpportunityFromInFront) probability += 2 + intelligence * 2
    if (hasRetreatOpportunityFromBehind) probability += intelligence * 2
    if (inFrontOnRampage) probability -= intelligence * 2
    if (behindOnRampage) probability -= intelligence * 2
  }

  function inFrontCloseAndBehindFar() {
    //console.log('inFrontCloseAndBehindFar');
    if (lowStamina) probability += intelligence * 2

    if (lowSpirit) probability += intelligence * 2

    if (notFullStamina) probability += intelligence

    if (notFullSpirit) probability += intelligence

    if (inFrontOnRampage) probability -= intelligence * 2
    if (behindOnRampage) probability -= intelligence
  }

  function inFrontFarAndBehindNear() {
    //console.log('inFrontFarAndBehindNear');

    if (lowStamina) probability += 2

    if (lowSpirit) probability += 2
    ...
    if (hasRetreatOpportunityFromBehind) {
      ...
      if (enemySpeed < 5 && fitness > 5) {
        if (lowStamina) probability += intelligence

        if (lowSpirit) probability += intelligence
      }
    } else {
      probability -= intelligence * 4
    }

    if (inFrontOnRampage) probability -= 2 + intelligence
    if (behindOnRampage) probability -= intelligence * 2
  }
  ...
  function inFrontFarAndBehindClose() {
    ...
    if (!hasRetreatOpportunityFromInFront) probability -= intelligence
  ...
  function inFrontFar() {
    ...
    if (notFullStamina) probability += 6 + intelligence * 4

    if (notFullSpirit) probability += 6 + intelligence * 4

    if (hasRetreatOpportunityFromInFront) probability += intelligence * 3
  }
  function inFrontNear() {
    //console.log('inFrontNear');
    if (lowStamina) probability += 6 + intelligence * 2

    if (lowSpirit) probability += 6 + intelligence * 2

    if (notFullStamina && !logistics.isEnemyAttacking(enemyInFront!)) {
      probability += intelligence * 4
    }

    if (notFullStamina) probability += 4 + intelligence * 2

    if (notFullSpirit) probability += 4 + intelligence * 2
  }
  ...
  function behindFar() {
    ...
    if (lowStamina) probability += 2 + intelligence * 4
    if (lowSpirit) probability += 2 + intelligence * 4

    if (notFullStamina && !logistics.isEnemyAttacking(enemyInFront!)) {
      probability += intelligence * 4
    }

    if (notFullStamina) probability += 2 + intelligence * 3

    if (notFullSpirit) probability += 2 + intelligence * 3
  }
  function behindNear() {
    //console.log('behindNear');
    probability -= 2 + intelligence
    if (lowStamina) probability -= 2 + intelligence
    if (lowSpirit) probability -= 2 + intelligence
  ...
  function inFrontFarAndDontKnowBehind() {
    ...
    probability += 6 - intelligence * 4
    if (lowStamina) probability += 6 - intelligence * 4

    if (lowSpirit) probability += 6 - intelligence * 4

    if (notFullStamina) probability += 2 - intelligence * 4

    if (notFullSpirit) probability += 2 - intelligence * 4

    if (hasRetreatOpportunityFromInFront) probability += 2 - intelligence * 4
    else probability -= intelligence * 4

    if (inFrontOnRampage) probability -= intelligence
  }
  function inFrontNearAndDontKnowBehind() {
    //console.log('inFrontNearAndDontKnowBehind');
    probability += 4 - intelligence * 5
    if (lowStamina) probability += 4 - intelligence * 5

    if (lowSpirit) probability += 4 - intelligence * 5

    if (hasRetreatOpportunityFromInFront) probability += 2 - intelligence * 5
    else probability -= intelligence * 5

    if (inFrontOnRampage) probability -= intelligence * 5
  }
  function inFrontCloseAndDontKnowBehind() {
    //console.log('inFrontCloseAndDontKnowBehind');
    probability -= 4 + intelligence * 8
    if (inFrontOnRampage) probability -= intelligence * 6
  }
