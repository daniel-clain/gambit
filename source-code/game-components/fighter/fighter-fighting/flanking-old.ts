...
export default class Flanking {
  ...
  determineIfFlanked() {
    ...
    if(sortedEnemies.length >= 2){
      const pairCriticality = this.isFlankedBy2Enemies(sortedEnemies[0], sortedEnemies[1])
      if(pairCriticality){
        flankingPairs.push({flanker1: sortedEnemies[0], flanker2: sortedEnemies[1]})
        criticality += pairCriticality
      }   
    } 
    
    if(sortedEnemies.length >= 3){
      {
        const pairCriticality = this.isFlankedBy2Enemies(sortedEnemies[0], sortedEnemies[2])
        if(pairCriticality){
      ...
      {
        const pairCriticality = this.isFlankedBy2Enemies(sortedEnemies[1], sortedEnemies[2])
        if(pairCriticality){
