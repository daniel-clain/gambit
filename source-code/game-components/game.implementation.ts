...
export class Game_Implementation{
  ...
  getAllConnectedGameDisplays(){
    ...
    return this.game.has.gameDisplays?.filter(gameDisplay => this.game.has.connectionManager.disconnectedClients
      .find(disconnectedClient => disconnectedClient.id == gameDisplay.id))
  }
