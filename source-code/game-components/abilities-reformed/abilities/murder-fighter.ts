...
export const murderFighterServer: ServerAbility = {
  execute(abilityData: AbilityData, game: Game){
    ...
    if(fighter.state.dead){
      hitmansManager.functions.addToLog({message: `Attempt to murder ${abilityData.target.name} failed beacuse he was already found dead`})
      return
    }
    ...
    if(success){
      ...
      hitmansManager.has.postFightReportItems.push({
