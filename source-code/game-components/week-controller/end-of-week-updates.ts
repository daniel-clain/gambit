...
export function doEndOfWeekUpdates(game: Game) {
  ...
  managers.forEach((manager) => {
    manager.has.nextFightBet = undefined
    manager.state.readyForNextFight = false
    manager.state.beingProsecuted = false
    manager.state.underSurveillance = undefined
    ...
    if (manager.state.inJail) {
      if ((manager.state.inJail.weeksRemaining = 1))
        manager.state.inJail = undefined
  ...
  function returnProfessionalJobSeekersToProfessionalsPool() {
    ...
    fighters.forEach((f) => {
      const isAJobSeeker = weekController.jobSeekers.some(
        (j) => j.name == f.name
      )
      if (isAJobSeeker) {
  ...
  function reduceExtremeFighterStats() {
    ...
    function reduceStat(stat: number) {
      if (stat > 8 && stat <= 10) return Math.round(stat - (stat - 8) * 0.2)
      if (stat > 10) return Math.round(stat - (stat - 10) * 0.5)
  ...
  function returnEmployeesAndFightersWithExpiredContracts(manager: Manager) {
    ...
    const expiredFighters: Fighter[] = manager.has.fighters.filter(
      (fighter) => fighter.state.activeContract?.weeksRemaining == 0
    )

    manager.has.fighters = manager.has.fighters?.filter(
      (fighter) => fighter.state.activeContract?.weeksRemaining != 0
    )

    expiredFighters?.forEach(
      (fighter) => (fighter.state.activeContract = undefined)
    )
  ...
  function payEmployeeAndFighterWages(manager: Manager) {
    ...
    if (allExpenses)
      manager.functions.addToLog({
        weekNumber,
        message: `Spent ${allExpenses} on employee wages`,
        type: "report",
      })
  }
  ...
  function updateEmployeeAndFighterWeeksLeft(manager: Manager) {
    ...
    fighters.forEach((fighter) => {
      ...
      if (fighter.state.activeContract!.weeksRemaining == 0) {
        ...
        manager.functions.addToLog({
          weekNumber,
          message: `Your fighter ${fighter.name}'s contract expires after this week. You must recontract him if you want him to stay`,
          type: "critical",
        })
      }
    })
  ...
  function updateLoanSharkData(manager: Manager) {
    const {
      minimumAmountToPayBackEachWeek,
      interestAddedPerWeek,
      weeksOfNoPaybackUntilRespond,
    } = gameConfiguration.loanSharkSettings
    ...
    if (manager.has.loan?.debt) {
      ...
      if (
        manager.has.loan.isNew ||
        managerHasPaidBackTheMinimumAmountThisWeek()
      ) {
        manager.has.loan = {
          ...manager.has.loan,
          weeksOverdue: 0,
      ...
      } else if (dayBeforeOverdue()) sendWarning()
    ...
    function sendWarning() {
      manager.functions.addToLog({
        weekNumber,
        type: "critical",
        message: `You have not made a repayment in ${
          manager.has.loan!.weeksOverdue
        } weeks, the loan shark is becoming impatient`,
      })
    }
    ...
    function managerHasPaidBackTheMinimumAmountThisWeek(): boolean {
      return (
        manager.has.loan!.amountPaidBackThisWeek >=
        minimumAmountToPayBackEachWeek
      )
    }
    ...
    function addInterestToLoan() {
      const addedAmount = Math.round(
        manager.has.loan!.debt * interestAddedPerWeek
      )
      manager.has.loan!.debt += addedAmount
      manager.functions.addToLog({
        weekNumber,
        message: `You loan debt has increased by ${addedAmount} because of ${
          interestAddedPerWeek * 100
        }% interest, you now owe ${manager.has.loan!.debt}`,
        type: "report",
      })
    }
    ...
    function loanSharkAssaultsOneOfManagersFighters() {
      if (manager.has.fighters.length == 0) return
      ...
      manager.functions.addToLog({
        weekNumber,
        type: "critical",
        message: `The loan shark is mad that you didn't pay him back, so he has abducted ${
          randomFighter.name
        } and tortured him, his ${randomStat.replace(
          "base",
          ""
        )} has been reduced by ${decrement}`,
      })
    }
  }
}
