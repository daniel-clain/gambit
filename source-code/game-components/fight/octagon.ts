...
export default class Octagon {
  ...
  static checkIfFighterIsNearEdge(baseDimensions: Dimensions, coords: Coords): Edge{
    ...
    for(let edgeKey of edgeKeys){
      ...
      switch(edgeKey){
        ...
        case 'top': {
          point = {...coords}
          point.y += baseDimensions.height
        }; break;
        case 'topRight':
        case 'right': {
          point = {...coords}
          point.y += baseDimensions.height
          point.x += baseDimensions.width
        }; break;
        case 'bottomRight': {
          point = {...coords}
          point.x += baseDimensions.width
  ...
  private static checkInsideEdgeForPoint(edgeKey: Edge, edgePoints, testPoint: Coords, close?: boolean): boolean{
